...
antenna_spacing = 0.5 #?
lambda_s = 1 #?
d = antenna_spacing / lambda_s
angles = np.linspace(-90, 90, 360)

def steering_vector(theta):
    # 计算波束形成向量
    index = np.arange(M)
    angles = 2 * np.pi * d * index * np.sin(np.radians(theta)) / lambda_s
    return np.exp(1j * angles)

def beampattern(theta, W):
    # 计算给定角度的波束模式
    a = steering_vector(theta)
    return np.abs(np.conj(a).T @ W @ W.conj().T @ a)
...
def calculate_sum_rate(hk, power_range):
    ...
    for power_dbm in power_range:
            ...
            for k in range(K):
                ...
                for j in range(K):
                    if j != k:
                        ...
                        x = A_vec @ W_combined_0[:, j]
                        Pattern_A += cp.square(cp.abs(x))
        ...
        # beampatterns_2 = []
        # for angle in angles:
        #     bp = beampattern(angle, W_combined_0)
        #     bp_2 = beampattern(angle, W_combined_0[:,K+1:])
        #     beampatterns.append(10 * np.log10(bp))
        #     beampatterns_2.append(10 * np.log10(bp_2))
        #
        # theta_list = [-35, 0, 35]
        # L = len(theta_list)
        # # 理想波束模式的基线（在目标方向为1，其余为0）
        # desired_beampattern = np.zeros_like(angles)
        # for theta in theta_list:
        #     desired_beampattern[np.abs(angles - theta) <= 5] = np.max(beampatterns)
        #
        # # 绘制波束模式图
        # plt.figure(figsize=(10, 6))
        # plt.plot(angles, beampatterns, label='Proposed w/ RIS', linestyle='--', color='red')
        # plt.plot(angles, beampatterns_2, label='Radar only', linestyle='--', color='blue')
        # plt.plot(angles, desired_beampattern, label='Desired', color='purple')
        # plt.xlabel('Direction (degrees)')
        # plt.ylabel('Beampattern (dBW)')
...
# Pd = []
# for k in range(L):
#     A.append(steering_vector(theta_list[k]))
#     Pd.append(beampattern(theta_list[k], W_combined_0))
